...
const CreationFlagValues : array [0..7] of Integer = (CREATE_DEFAULT_ERROR_MODE, CREATE_NEW_CONSOLE, CREATE_NEW_PROCESS_GROUP, CREATE_SEPARATE_WOW_VDM, CREATE_SUSPENDED, CREATE_UNICODE_ENVIRONMENT, DEBUG_PROCESS, DEBUG_ONLY_THIS_PROCESS);
...
begin //FExecute
  ...
  begin
    FileClose(newstdin); { *Converted from CloseHandle*  }
    FileClose(write_stdin); { *Converted from CloseHandle*  }
    raise FCreateProcessError;  //  æ‚É raise ‚µ‚Ä‚Í‚È‚ç‚È‚¢B
    Exit;
  end;
 ...
 validates the wShowWindow member.}
  ...
  begin
        //  —áŠO‚ðŽó‚¯Žæ‚éêŠ‚ª–³‚¢B
        //  ƒXƒŒƒbƒh‚Ì exitcode ‚É“ü‚ê‚é‚×‚«HB
  ...
  try /// handles of process
    ...
    repeat //main program loop
            //  ¶‚«‚Ä‚¢‚ê‚Î STILL_ACTIVE Ž€‚ñ‚Å‚¢‚ê‚ÎI—¹ƒR[ƒh‚ª‚½‚¾‚¿‚É•Ô‚éB
            //  Ž€‚ñ‚Å‚¢‚Ä‚àƒpƒCƒv‚Ìƒf[ƒ^‚Í—LŒøB
      GetExitCodeProcess(pi.hProcess, Exit_Code); //while the process is running
            //  o—ÍƒpƒCƒv‚©‚ç pBuf ‚ÉŽæ‚èž‚ÞB
      ...
      if (bread <> 0) then begin
        ...
        SysUtils.FileRead(read_stdout,pBuf,iBufSize); { *Converted from ReadFile*  } //read the stdout pipe
    ...
    FileClose(pi.hThread); { *Converted from CloseHandle*  }
    FileClose(pi.hProcess); { *Converted from CloseHandle*  }
  end;
  finally /// handles for pipes
    FileClose(newstdin); { *Converted from CloseHandle*  } //clean stuff up
    FileClose(newstdout); { *Converted from CloseHandle*  }
    FileClose(read_stdout); { *Converted from CloseHandle*  }
    FileClose(write_stdin); { *Converted from CloseHandle*  }
  end;
  finally /// memory(1)
    FreeMem(pBuf);
...
//------------------------------------------------------------------------------
    ...
    //
    //  AOwner:TDosCommand ‚ðˆê‚Â‚¾‚¯Žó‚¯Žæ‚Á‚½‚Ù‚¤‚ª‚æ‚¢B
    //  
...
//------------------------------------------------------------------------------
    ...
    //
    //  WaitFor ‚Å‚ÍeƒXƒŒƒbƒh‚ªŽ~‚Ü‚Á‚Ä‚µ‚Ü‚¤H
    //  
    //  Apollo ‚Å‚Í‹N“®’†‚É[‚˜]‚ÅI—¹‚µ‚Ä‚àAmainloop ‚ðI—¹‚µ‚È‚¢B
    //  
    //
